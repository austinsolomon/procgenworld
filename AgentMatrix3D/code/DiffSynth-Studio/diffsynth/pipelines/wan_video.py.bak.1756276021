...
class WanVideoPipeline(BasePipeline):
        ...
        # ok;
            ...
            try:
                y = self.vae.encode([vae_input.to(dtype=self.torch_dtype, device=self.device)],
                                    device=self.device, tiled=tiled,
                                    tile_size=tile_size, tile_stride=tile_stride)[0]
            except RuntimeError as e:
                # OOM fallback: force CPU + tight tiling, then move latent back to GPU.
                if "out of memory" not in str(e).lower():
                    raise
                import torch, gc
                torch.cuda.empty_cache(); gc.collect()
                vae_dev = torch.device("cpu")
                y = self.vae.encode([vae_input.to(dtype=self.torch_dtype, device=vae_dev)],
                                    device=vae_dev,
                                    tiled=True if tiled is None else tiled,
                                    tile_size=96 if tile_size is None else tile_size,
                                    tile_stride=72 if tile_stride is None else tile_stride)[0]
                y = y.to(self.device)
        except RuntimeError as e:
            # OOM fallback: empty cache, run VAE encode on CPU with tight tiling, then move back to GPU
            if "out of memory" not in str(e).lower():
                raise
            import torch, gc
            torch.cuda.empty_cache(); gc.collect()
            vae_dev = torch.device("cpu")
            y = self.vae.encode([vae_input.to(dtype=self.torch_dtype, device=vae_dev)],
                                device=vae_dev, tiled=True if tiled is None else tiled,
                                tile_size=96 if tile_size is None else tile_size,
                                tile_stride=72 if tile_stride is None else tile_stride)[0]
            y = y.to(self.device)
    ...
    def encode_image_with_mask(self, images, masks, num_frames, height, width, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
        
        image_ff = self.preprocess_image(images[0]).to(self.device)
        images = self.preprocess_images(images)#.to(self.device) # returns a list of [1,3,h,w] image;, range -1~1
        masks = self.preprocess_images(masks)
        
        clip_context = self.image_encoder.encode_image([image_ff])

        msk = torch.concat([(torch.nn.functional.interpolate((mask[:,:1,:,:] ),(height//8,width//8),mode = "bilinear") + 1.)/2. for mask in masks], dim=1).to(self.device) # [1,num_frames,h,w]
        
        vae_input = torch.concat([image.permute(1,0,2,3) for image in images], dim=1).to(self.device)
    ...
    def __call__(
        ...
        if input_image is not None and self.image_encoder is not None:
            ...
            image_emb = {}
        #print(image_emb.keys())
        #print(image_emb["y"])
        # add: encode video
        if cond_video is not None and cond_mask is not None and self.image_encoder is not None:
            self.load_models_to_device(["image_encoder", "vae"])
            # (self, images, masks, num_frames, height, width, tiled=False, tile_size=(34, 34), tile_stride=(18, 16)):
            image_emb = self.encode_image_with_mask(cond_video, cond_mask, num_frames, height, width, **tiler_kwargs)
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Inference
            #print(image_emb["y"].__class__)
            #print(image_emb["y"].shape)
            #print(image_emb["y"].shape)
